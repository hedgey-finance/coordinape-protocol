...
contract ApeDistributor is ApeAllowanceModule {
    ...
    mapping(address => mapping(bytes32 => mapping(address => mapping(uint256 => bytes32))))
        public epochRoots;
    mapping(bytes32 => mapping(address => uint256)) public epochTracking;
    mapping(address => mapping(bytes32 => mapping(address => mapping(uint256 => mapping(uint256 => uint256)))))
    ...
    function _tap(
        ...
        require(
            ApeVaultFactory(ApeRegistry(registry).factory()).vaultRegistry(_vault),
            "ApeDistributor: Vault does not exist"
        );
        bool isOwner = ApeVaultWrapperImplementation(_vault).owner() == msg.sender;
        require(
            vaultApprovals[_vault][_circle] == msg.sender || isOwner,
            "ApeDistributor: Sender not approved"
        );

        if (_tapType == uint8(2))
            require(
                address(ApeVaultWrapperImplementation(_vault).simpleToken()) == _token,
                "ApeDistributor: Vault cannot supply token"
            );
        else
            require(
                address(ApeVaultWrapperImplementation(_vault).vault()) == _token,
                "ApeDistributor: Vault cannot supply token"
            );

        if (!isOwner) _isTapAllowed(_vault, _circle, _token, _amount);
        ...
        require(
            afterBal - beforeBal == _amount,
            "ApeDistributor: Did not receive correct amount of tokens"
        );

        if (sharesRemoved > 0)
            emit yearnApeVaultFundsTapped(
                _vault,
                address(ApeVaultWrapperImplementation(_vault).vault()),
                sharesRemoved
            );
    ...
    function sum(uint256[] calldata _vals) internal pure returns (uint256 res) {
        for (uint256 i = 0; i < _vals.length; i++) res += _vals[i];
    }
    ...
    function tapEpochAndDistribute(
        ...
        if (_unlock > block.timestamp) {
			IERC20(_token).safeIncreaseAllowance(hedgeyNFT, _amount);
			for (uint i; i < _users.length; i++) {
				INFT(hedgeyNFT).createNFT(_users[i], _amounts[i], _token, _unlock);
			}
		} else {
    ...
    function claim(
        ...
        require(
            claimable <= circleAlloc[_vault][_circle][_token],
        ...
        if (_redeemShares && msg.sender == _account) VaultAPI(_token).withdraw(claimable, _account);
